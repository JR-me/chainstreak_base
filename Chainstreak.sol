...
contract Chainstreak is ERC721, Ownable {
    ...
    function _buildSVG(
        ...
        // Each helper kept deliberately small to stay within the EVM stack limit
        string memory part1 = _svgHeader(s);
        string memory part2 = _svgRings();
        string memory part3 = _svgTextTop(s, data);
        string memory part4 = _svgTextBottom(s, data, tokenId);

        return string(abi.encodePacked(part1, part2, part3, part4, '</svg>'));
    }
    ...
    // SVG header part A: opening tag + tier-coloured gradients (bg, rg, ng)
    ...
    // SVG header part B: fixed-colour gradients (sh, sw) + background shapes
    ...
    // Ring colours come from the #rg gradient defined in _svgHeader
    ...
    // Text top: streak number, BEST STREAK label, divider, tier name
    function _svgTextTop(
        TierStyle memory s,
        StreakData memory data
    ) internal pure returns (string memory) {
        return string(abi.encodePacked(
    ...
    // Text bottom: stats panel + token ID pill
    function _svgTextBottom(
        TierStyle memory s,
        StreakData memory data,
        uint256 tokenId
    ) internal pure returns (string memory) {
